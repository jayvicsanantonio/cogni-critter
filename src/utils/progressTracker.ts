...
export function getProgressMilestones(
  ...
  return [
    ...
    },
    {
      ...
      label: 'Ready to Test',
      description:
        'Your critter has enough examples to start testing!',
    ...
    },
    {
      ...
      label: 'Expert Level',
      description:
        'Your critter has learned from the maximum examples!',
 ...
 */
export function analyzeTrainingQuality(
  ...
  if (total < 5) {
    suggestions.push(
      `Add ${
        5 - total
      } more examples to reach the minimum for testing`
    );
  }
...
export class ProgressTracker {
  ...
  getSessionStats(): {
    ...
    const exampleEvents = this.events.filter(
      (e) => e.type === 'example_added'
    );
    ...
    return {
      ...
      averageTimePerExample:
        exampleEvents.length > 0
          ? sessionDuration / exampleEvents.length
          : 0,
    };
  }
...
export function getProgressColor(
  ...
  if (current >= maximum)
    return PROGRESS_VISUAL_CONFIG.colors.maximum;
  if (current >= minimum)
    return PROGRESS_VISUAL_CONFIG.colors.complete;
