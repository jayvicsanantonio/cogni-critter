...
describe('MemoryManager', () => {
  const mockMemory = tf.memory as jest.MockedFunction<typeof tf.memory>
  const mockDisposeVariables = tf.disposeVariables as jest.MockedFunction<
  ...
  describe('Garbage Collection', () => {
    ...
    it('should handle WebGL backend cleanup', () => {
      ...
      ;(tf.backend as jest.MockedFunction<typeof tf.backend>).mockReturnValue(
        mockBackend
      )
  ...
  describe('Cleanup Callbacks', () => {
    it('should register and execute cleanup callbacks', () => {
      ...
      const _unregister2 = memoryManager.registerCleanupCallback(callback2)
