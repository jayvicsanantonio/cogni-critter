...
export function calculateEmotionalContext(
  ...
  if (testResults.length >= 4) {
    ...
    const secondHalfAccuracy =
      secondHalf.filter((r) => r.isCorrect).length /
      secondHalf.length;
 ...
 */
export function determineCritterMood(
...
export class CritterEmotionalStateManager {
  ...
  private callbacks: Map<string, (mood: CritterMood) => void> =
    new Map();
  ...
  updateMood(testResults: TestResult[]): CritterMood {
    ...
    if (
      ...
      // Trigger callbacks
      this.callbacks.forEach((callback) => callback(newMood));
    }
  ...
  handlePredictionResult(
    ...
    // Trigger callbacks
    this.callbacks.forEach((callback) => callback(predictionMood));
  ...
  getMoodStats() {
    ...
    const moodCounts = this.moodHistory.reduce((counts, mood) => {
    ...
    for (let i = 1; i < this.moodHistory.length; i++) {
      if (
        this.moodHistory[i].state !== this.moodHistory[i - 1].state
      ) {
        moodChanges++;
      }
    }
 ...
 * Singleton instance
 */
export const critterEmotionalStateManager =
  new CritterEmotionalStateManager();
