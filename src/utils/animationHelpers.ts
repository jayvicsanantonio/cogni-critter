 ...
 * Provides utilities for creating smooth, performant animations
 */
  ...
  static monitorPerformance = {
    ...
    reportFrameDrop(): void {
      ...
      if (__DEV__ && this.frameDrops % 10 === 0) {
        console.warn(
          `[Animation] Frame drops detected: ${this.frameDrops}`
        );
      }
    },
...
export const useAnimationMonitoring = (animationName: string) => {
  const startAnimation = () => {
    AnimationHelper.monitorPerformance.startMonitoring(animationName);
  };

  const endAnimation = (duration: number) => {
    AnimationHelper.monitorPerformance.endMonitoring(
      animationName,
      duration
    );
  };
