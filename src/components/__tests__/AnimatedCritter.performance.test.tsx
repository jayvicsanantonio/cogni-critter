...
describe('AnimatedCritter Performance Tests', () => {
  ...
  it('should maintain 60fps during state transitions', async () => {
    ...
    const stats = await performanceMonitor.testAnimationPerformance(3000)
  ...
  it('should maintain acceptable performance with rapid state changes', async () => {
    const TestComponent = () => {
      ...
      React.useEffect(() => {
        ...
        const states: CritterState[] = ['IDLE', 'THINKING', 'HAPPY', 'CONFUSED']
        let currentIndex = 0
    ...
    const stats = await performanceMonitor.testAnimationPerformance(2000)
  ...
  it('should handle multiple AnimatedCritter instances efficiently', async () => {
      ...
      return (
        <>
          {states.map((state, index) => (
            <AnimatedCritter
              key={`critter-${state}-${Date.now()}-${Math.random()}`}
              state={state}
              critterColor={['Cogni Green', 'Spark Blue', 'Glow Yellow'][index]}
              animationDuration={250}
            />
          ))}
        </>
    ...
    const stats = await performanceMonitor.testAnimationPerformance(2500)
  ...
  it('should provide performance recommendations when needed', async () => {
    ...
    const stats = await performanceMonitor.testAnimationPerformance(1000)
  ...
  it('should track frame drops accurately', async () => {
    ...
    const stats = await performanceMonitor.testAnimationPerformance(1500)
 ...
 */
const _ManualPerformanceTest: React.FC = () => {
  const [currentState, setCurrentState] = React.useState<CritterState>('IDLE')
  const [_isRunning, setIsRunning] = React.useState(false)
  const [_stats, setStats] = React.useState<PerformanceStats | null>(null)

  const _runPerformanceTest = async () => {
    setIsRunning(true)
    setStats(null)
