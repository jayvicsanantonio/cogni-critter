...
describe('State Validation', () => {
  it('should validate correct transitions', () => {
    expect(isValidTransition('INITIALIZING', 'LOADING_MODEL')).toBe(
      true
    );
    expect(isValidTransition('LOADING_MODEL', 'TEACHING_PHASE')).toBe(
      true
    );
    expect(isValidTransition('TEACHING_PHASE', 'TESTING_PHASE')).toBe(
      true
    );
    expect(
      isValidTransition('TESTING_PHASE', 'RESULTS_SUMMARY')
    ).toBe(true);
    expect(
      isValidTransition('RESULTS_SUMMARY', 'TEACHING_PHASE')
    ).toBe(true);
  });

  it('should reject invalid transitions', () => {
    expect(isValidTransition('INITIALIZING', 'TESTING_PHASE')).toBe(
      false
    );
    expect(
      isValidTransition('TEACHING_PHASE', 'RESULTS_SUMMARY')
    ).toBe(false);
    expect(
      isValidTransition('LOADING_MODEL', 'RESULTS_SUMMARY')
    ).toBe(false);
  });
});
...
describe('Training Data Management', () => {
  ...
  it('should determine when to transition to testing', () => {
    ...
    expect(shouldTransitionToTesting(trainingData.slice(0, 3))).toBe(
      false
    );
  });
});
...
describe('Critter State Integration', () => {
  it('should map phases to critter states correctly', () => {
    ...
    expect(getCritterStateForPhase('LOADING_MODEL')).toBe(
      'LOADING_MODEL'
    );
