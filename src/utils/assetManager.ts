...
export class AssetManager {
  ...
  private loadingPromises: Map<string, Promise<CachedImage>> = new Map()
   ...
   */
  public async getTeachingImages(count: number = 8): Promise<ImageItem[]> {
    if (!this.dataset) {
      throw new Error('AssetManager not initialized. Call initialize() first.')
    }
   ...
   * @returns Array of mixed apple and non-apple images
   */
  public async getTestingImages(count: number = 5): Promise<ImageItem[]> {
    if (!this.dataset) {
      throw new Error('AssetManager not initialized. Call initialize() first.')
    }
  ...
  private cleanupCache(): void {
    ...
    if (removedCount > 0) {
      console.log(`AssetManager: Cleaned up ${removedCount} old cache entries`)
    }
