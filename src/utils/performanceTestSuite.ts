...
export class PerformanceTestSuite {
  ...
  public async runFullTestSuite(): Promise<PerformanceTestReport> {
    ...
    for (const scenario of scenarios) {
      ...
      try {
        ...
        console.log(
          `${status} - ${scenario.name} (Score: ${result.score})`
        );
      } catch (error) {
        console.error(
          `💥 Test failed with error: ${scenario.name}`,
          error
        );
        testResults.push({
    ...
    return [
      {
        ...
        description:
          'Test sustained 60fps performance during animations',
        duration: 5000,
        test: async () => {
      ...
      },
      {
        ...
        test: async () => {
          ...
          const memoryGrowth =
            finalSnapshot.numBytes - snapshots[0].numBytes;
        ...
        test: async () => {
          ...
          for (let i = 0; i < 10; i++) {
            ...
            if (predictionTime > 1000) {
              throw new Error(
                `Prediction too slow: ${predictionTime}ms`
              );
            }
          }
  ...
  private async runTestScenario(
    ...
    try {
      ...
      recommendations.push(
        'Investigate test failure and optimize performance'
      );
    }
    ...
    if (performanceStats.averageFPS < 45) {
      ...
      issues.push(
        `Low frame rate: ${performanceStats.averageFPS}fps`
      );
      recommendations.push(
        'Optimize animations and reduce complexity'
      );
    }
  ...
  private generateTestReport(
    ...
    testResults.forEach((result) => {
      if (!result.passed) {
        criticalIssues.push(
          `${result.scenario}: ${result.issues.join(', ')}`
        );
      }
...
export const performanceTestSuite =
  PerformanceTestSuite.getInstance();
