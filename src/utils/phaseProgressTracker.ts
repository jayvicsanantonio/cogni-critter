...
export function calculateTeachingProgress(
  ...
  const balanceRatio =
    current > 0 ? Math.abs(0.5 - appleCount / current) : 0;
 ...
 */
export function getTeachingProgressMessage(
 ...
 */
export function getTestingProgressMessage(
...
export class EnhancedProgressTracker {
  ...
  private callbacks: Map<string, (progress: PhaseProgress) => void> =
    new Map();
  ...
  onProgress(
    id: string,
    callback: (progress: PhaseProgress) => void
  ) {
