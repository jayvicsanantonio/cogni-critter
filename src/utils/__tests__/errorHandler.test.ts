...
import { errorHandler, GameError } from '../errorHandler'
...
describe('ErrorHandler', () => {
  ...
  describe('Model Loading Errors', () => {
    ...
    it('should provide user-friendly message for model loading errors', () => {
      ...
      const gameError = errorHandler.handleModelLoadError(originalError)
  ...
  describe('Memory Errors', () => {
    it('should handle memory errors', () => {
      ...
      const gameError = errorHandler.handleMemoryError(originalError, context)
  ...
  describe('App State Errors', () => {
    it('should handle app state change errors', () => {
      ...
      const gameError = errorHandler.handleAppStateError(originalError, context)
  ...
  describe('Tensor Errors', () => {
    it('should handle tensor operation errors', () => {
      ...
      const gameError = errorHandler.handleTensorError(originalError, context)
  ...
  describe('Recovery Suggestions', () => {
    it('should provide recovery suggestions for model loading errors', () => {
      const gameError = errorHandler.handleModelLoadError(new Error('Test'))
      const suggestions = errorHandler.getRecoverySuggestions(gameError)
    ...
    it('should provide recovery suggestions for prediction timeouts', () => {
      ...
      expect(suggestions).toContain('The image will be skipped automatically')
      expect(suggestions).toContain('Try with a different image')
    })

    it('should provide recovery suggestions for memory errors', () => {
      const gameError = errorHandler.handleMemoryError(new Error('Test'))
      const suggestions = errorHandler.getRecoverySuggestions(gameError)

      expect(suggestions).toContain('Close other apps to free up memory')
  ...
  describe('User-Friendly Error Display', () => {
    it('should show retryable errors with retry option', () => {
      ...
      const gameError = errorHandler.handleModelLoadError(new Error('Test'))
      errorHandler.showUserFriendlyError(gameError, mockOnRetry, mockOnCancel)
    ...
    it('should show non-retryable errors with OK button only', () => {
      ...
      const gameError = errorHandler.handleMemoryError(new Error('Test'))
      errorHandler.showUserFriendlyError(gameError, undefined, mockOnCancel)
  ...
  describe('Error Classification', () => {
    it('should correctly identify recoverable errors', () => {
      ...
      expect(errorHandler.isRecoverable(nonRecoverableError)).toBe(true) // UNKNOWN_ERROR is recoverable
    })
