...
export class PerformanceMetricsCollector {
  ...
  public generateReport(testDuration?: number): PerformanceReport {
    ...
    if (!currentMetrics.validation.meetsMemoryLimits) {
      issues.push(
        `Memory usage exceeds limits: ${(
          currentMetrics.memory.bytes /
          1024 /
          1024
        ).toFixed(1)}MB`
      );
      recommendations.push(
        'Implement tensor disposal and memory cleanup'
      );
    }
    ...
    if (
      modelLoadTime &&
      modelLoadTime > this.criteria.maxModelLoadTime
    ) {
    ...
    if (
      predictionTime &&
      predictionTime > this.criteria.maxPredictionTime
    ) {
      issues.push(`Prediction time too slow: ${predictionTime}ms`);
      recommendations.push(
        'Optimize image preprocessing or model size'
      );
    }
  ...
  private collectMetrics(): PerformanceMetrics {
    ...
    // Calculate overall score
    metrics.validation.overallScore =
...
export const performanceMetrics =
  PerformanceMetricsCollector.getInstance();
...
export const usePerformanceMetrics = () => {
  ...
  const validate60FPS = (duration?: number) =>
    collector.validate60FPS(duration);
