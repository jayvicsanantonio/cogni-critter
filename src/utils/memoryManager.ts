...
export class MemoryManager {
  ...
  public initialize(customThresholds?: Partial<MemoryThresholds>): void {
    ...
    console.log('Memory manager initialized with thresholds:', this.thresholds)
  }
  ...
  public takeSnapshot(context?: string): MemorySnapshot {
    ...
    if (this.snapshots.length > this.SNAPSHOT_HISTORY_SIZE) {
      this.snapshots = this.snapshots.slice(-this.SNAPSHOT_HISTORY_SIZE)
    }
   ...
   */
  public safeTensorDispose(tensor: tf.Tensor | null | undefined): void {
    if (!tensor) {
      return
    }
   ...
   */
  public registerAlertCallback(callback: MemoryAlertCallback): () => void {
  ...
  public getDetailedMemoryStats(): {
    ...
    const recommendations = this.generateMemoryRecommendations(current, usage)
  ...
  private logMemoryDelta(
    ...
    if (delta.tensors > 0 || Math.abs(delta.bytes) > 1024 * 1024) {
      ...
      console.log(`Memory delta ${context || ''}:`, {
        tensors: delta.tensors > 0 ? `+${delta.tensors}` : delta.tensors,
        bytes: `${delta.bytes > 0 ? '+' : ''}${(
          delta.bytes / 1024 / 1024
        ).toFixed(2)}MB`,
  ...
  private performBackendSpecificCleanup(): void {
    ...
    try {
      if (backend === 'webgl') {
        const webglBackendUnknown = tf.backend() as unknown

        // Check for WebGL-specific cleanup methods
        const maybeBackend = webglBackendUnknown as {
          checkCompileCompletion?: unknown
          getUniformLocations?: unknown
        }
        ...
        if (
          maybeBackend &&
          typeof maybeBackend.getUniformLocations === 'function'
        ) {
          // Clear uniform location cache if available
          const locations = (
            maybeBackend.getUniformLocations as () => unknown
          )()
          if (
            locations &&
            typeof (locations as { clear?: unknown }).clear === 'function'
          ) {
            ;(locations as { clear: () => void }).clear()
          }
        }
      }
    } catch (error) {
