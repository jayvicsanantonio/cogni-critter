...
export function evaluateTeachingTransition(
  ...
  if (progress.isMaximumReached) {
    return {
      ...
      delay: config.autoTransitionEnabled ? config.transitionDelay : undefined,
    }
  }
  ...
  if (progress.isMinimumReached) {
    ...
    const validation = validateTrainingDataForTransition(trainingData)
 ...
 */
function validateTrainingDataForTransition(trainingData: TrainingExample[]): {
  isValid: boolean
  issues: string[]
  suggestions: string[]
} {
  ...
  if (total > 0) {
    ...
    if (appleRatio > 0.9) {
      issues.push('Training data is too heavily skewed toward apples')
      suggestions.push('Add more non-apple examples for better balance')
    } else if (appleRatio < 0.1) {
      issues.push('Training data is too heavily skewed toward non-apples')
...
export class PhaseTransitionManager {
   ...
   */
  setTransitionCallback(callback: (nextPhase: GamePhase) => void): void {
   ...
   */
  evaluateTransition(trainingData: TrainingExample[]): TransitionResult {
    const result = evaluateTeachingTransition(trainingData, this.config)
    ...
    if (result.shouldTransition && result.delay && this.onTransitionCallback) {
      this.transitionTimer = setTimeout(async () => {
        ...
        if (result.requiresTraining && this.onTrainingCallback) {
          const trainingSuccess = await this.onTrainingCallback(trainingData)
          if (trainingSuccess) {
            ...
            console.error('Training failed during automatic transition')
          }
        } else {
   ...
   */
  async forceTransition(trainingData: TrainingExample[]): Promise<boolean> {
    const result = evaluateTeachingTransition(trainingData, this.config)
 ...
 */
export function createPhaseTransitionHook(config?: Partial<TransitionConfig>) {
  ...
  return {
    ...
    setupAutoTransition: (
      ...
      onTraining?: (trainingData: TrainingExample[]) => Promise<boolean>
    ) => {
...
export const transitionUI = {
  ...
  getTransitionMessage: (result: TransitionResult): string => {
    if (result.shouldTransition && result.delay) {
      return `${result.reason} Transitioning to testing in ${Math.ceil(
        result.delay / 1000
      )} seconds...`
    }
  ...
  getTransitionButtonText: (result: TransitionResult): string => {
    ...
    return result.requiresTraining ? 'Train Critter' : 'Start Testing'
  },
}
