...
export class ScorePersistenceService {
  ...
  static getInstance(): ScorePersistenceService {
    if (!ScorePersistenceService.instance) {
      ScorePersistenceService.instance =
  ...
  createSession(
    ...
    const accuracy =
      testResults.length > 0
        ? correctAnswers / testResults.length
        : 0;
    ...
    return {
      id: `session_${endTime}_${Math.random()
        .toString(36)
        .substr(2, 9)}`,
      timestamp: endTime,
      duration,
  ...
  private async updateStats(sessions: GameSession[]): Promise<void> {
    ...
    const totalTests = sessions.reduce(
    ...
    const overallAccuracy =
      totalTests > 0 ? totalCorrect / totalTests : 0;

    const bestAccuracy = Math.max(...sessions.map((s) => s.accuracy));
    const bestStreak = Math.max(
      ...sessions.map((s) => s.longestStreak)
    );
    ...
    if (sessions.length >= 10) {
      ...
      const recentAvg =
        recent5.reduce((sum, s) => sum + s.accuracy, 0) / 5;
      const previousAvg =
        previous5.reduce((sum, s) => sum + s.accuracy, 0) / 5;

      improvementTrend = recentAvg - previousAvg;
    }

    // Find most used critter color
    const colorCounts = sessions.reduce((counts, session) => {
      counts[session.critterColor] =
        (counts[session.critterColor] || 0) + 1;
      return counts;
 ...
 * Singleton instance
 */
export const scorePersistenceService =
  ScorePersistenceService.getInstance();
