...
/**
 * Color Tinting Manager
 * Provides utilities for applying color tints to grayscale sprites
 */
export class ColorTintingManager {
  /**
  ...
  static getColorValue(colorName: string): string {
    ...
    const color =
      CritterColors[colorName as keyof typeof CritterColors];
    if (color) {
      return color;
    }
   ...
   */
  static getAllCritterColors(): Array<{
    name: CritterColor;
    hex: string;
  }> {
   ...
   */
  static applyOpacity(hexColor: string, opacity: number): string {
    // Remove # if present
